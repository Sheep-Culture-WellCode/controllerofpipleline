...
public class CoordinatesChangeUtil {
    public static CDChange changetest(String XOC, String YOC){
        CDChange cdChange = new CDChange();
        double x,y,B,L,L0=0;
        double X,Y;
        X=Double.parseDouble(XOC);
        Y=Double.parseDouble(YOC);
        x = X;
        y = Y;
...
//        System.out.println("纬度："+B+"    经度:"+L+"    中央子午线："+L0);
        cdChange.setX(String.valueOf(B));
        cdChange.setY(String.valueOf(L));
        return cdChange;
    }
